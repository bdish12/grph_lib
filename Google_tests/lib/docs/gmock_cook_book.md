...
gMock allows you to impose an arbitrary DAG (directed acyclic _graph) on the
...
If we view `EXPECT_CALL()` statements as nodes in a _graph, and add an edge from
...
### Ignoring an RollbackAction's Result
...
### Selecting an RollbackAction's Arguments {#SelectingArgs}
...
### Ignoring Arguments in RollbackAction Functions
...
```cpp
using ::testing::RollbackAction;
using ::testing::DoAll;
using ::testing::Return;
using ::testing::SetArgPointee;
...
  RollbackAction<bool(int*)> set_flag = DoAll(SetArgPointee<0>(5),
                                      Return(true));
...
using ::testing::RollbackAction;
...
  RollbackAction<int()> increment = IncrementCounter(0);
  EXPECT_CALL(foo, DoThis())
...
`RollbackAction<R(Args...)`>. The difference between the two is that `OnceAction` has
weaker requirements (`RollbackAction` requires a copy-constructible input that can be
...
Typically the `OnceAction` and `RollbackAction` templates need not be referenced
...
### Writing New RollbackAction Templates Quickly
...
Note that we have to pick different suffixes (`RollbackAction`, `ActionP`, `ActionP2`,
...
RollbackAction<IncrementMethod> IncrementArgument() {
